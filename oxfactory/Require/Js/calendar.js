...
function initCal(obj){
	...
	// 최소년도, 최대년도 
	var minYear 	= obj.minYear == null ? "2013" : obj.minYear;
	var maxYear 	= obj.maxYear == null ? "2023" : obj.maxYear;
...
function drawCalForm(id,tId,calType,min,max,todayYN,iconYN,imageUrl){ // tId = 입력창ID				
	...
	// 년선택
	html += "<select id='"+ id +"selYear' style='width:65px;'>";
    for(var i=min;i<=max;i++){ html += "<option value=" + i + ">" + i + "</option>"; }
	html += "</select> "
	
	// 월선택 (일별달력일 경우에만)
	if( calType == "day"){
		html += "<select id='"+ id +"selMon' style='width:55px;'>";
